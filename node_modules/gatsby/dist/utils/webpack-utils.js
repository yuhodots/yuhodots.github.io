...
exports.ensureRequireEslintRules = ensureRequireEslintRules;
...
function ensureRequireEslintRules(config) {
  if (!config.module) {
    config.module = {
      rules: []
    };
  ...
  const rule = config.module.rules.find(rule => {
    if (typeof rule.loader === `string`) {
      return rule.loader === `eslint-loader` || rule.loader.endsWith(`eslint-loader/index.js`) || rule.loader.endsWith(`eslint-loader/dist/cjs.js`);
    }
  ...
  if (rule) {
    if (typeof rule.options !== `string`) {
      ...
      (0, _eslintConfig.mergeRequiredConfigIn)(rule.options);
    }
  } else {
